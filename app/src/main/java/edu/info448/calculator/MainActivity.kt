...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        divButton.setOnClickListener {
            try {
                compute(Calculator.Operator.DIV)  // first bug: it should be DIV rather than MUL
        ...
        resetButton.setOnClickListener { mResultTextView!!.setText(R.string.initial_result_text)
                                         mCalculator!!.setValue(0.0)} // second bug: when RESET button is clicked, the value should also be reset to be 0.0
    }
    ...
    private fun compute(operator: Calculator.Operator) {
        ...
        when (operator) {
            ...
            Calculator.Operator.SUB -> result = mCalculator!!.sub(operand).toString() // third bug: should do real SUB thing rather than error
